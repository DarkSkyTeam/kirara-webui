...
def execute(${props.selectedNode?.data.inputs?.map((input: { name: string, type: string }) =>
                    `${input.name}: ${input.type}`).join(', ') || ''}) -> Dict[str, Any]:
    # ... 处理数据 ...
    # 设置输出
    result = {
        ${props.selectedNode?.data.outputs?.map((output: { name: string, type: string }) =>
                    `"${output.name}": None # 请修改为实际结果`).join(',\n        ') || ''}
    }
...
const inputConnections = computed(() => {
    ...
    return edges
        ...
        .map(edge => {
            ...
            const sourceHandle = sourceNode?.data.blockType.outputs.find((output: { name: string }) => output.name === edge.sourceHandle)
                || sourceNode?.data.outputs?.find((output: { name: string }) => output.name === edge.sourceHandle)
            return {
                ...edge,
...
const outputConnections = computed(() => {
    ...
    return edges
        ...
        .map(edge => {
            ...
            const targetHandle = targetNode?.data.blockType.inputs.find((input: { name: string }) => input.name === edge.targetHandle)
...
const addInputPort = () => {
    if (props.selectedNode && isCodeNode.value) {
        const updatedData = {
            ...props.selectedNode.data,
            inputs: [...(props.selectedNode.data.inputs || []), {
...
const addOutputPort = () => {
    if (props.selectedNode && isCodeNode.value) {
        const updatedData = {
            ...props.selectedNode.data,
            outputs: [...(props.selectedNode.data.outputs || []), {
...
const handleEditorMount = ({ editor, lspClient }: { editor: any, lspClient: MonacoLanguageClient }) => {
...
const updateLspMandatoryFunction = () => {
    ...
    const newSignature = `def execute(${props.selectedNode.data.inputs?.map((input: { name: string, type: string }) =>
        `${input.name}: ${input.type}`).join(', ') || ''}) -> Dict[str, Any]:`;
...
${newSignature}
    ...
    lspClientRef.value.sendNotification('workspace/didChangeConfiguration', {
        settings: {
            'mandatoryFunction': {
                "name": "execute",
                "params": props.selectedNode.data.inputs?.map((input: { name: string, type: string }) => ({
                    "name": input.name,
...
<template>
    <NCard title="节点配置" size="small" class="node-config-panel" :bordered="false" content-style="height: 100%;">
        <template #header>
        ...
        <NScrollbar style="height: 100%;">
            ...
            <div v-if="selectedNode" class="config-content">
                ...
                <div class="node-description">
                    <NText class="node-description-value">{{ selectedNode.data.blockType.description }}</NText>
                </div>
                ...
                <NCollapse arrow-placement="right" :default-expanded-names="[1, 2, 4]">
                    ...
                    <NCollapseItem title="输入连接" name="1">
                        ...
                        <div v-if="selectedNode">
                            <div v-for="input in selectedNode.type === 'code' ? selectedNode.data.inputs : selectedNode.data.blockType.inputs"
                                :key="input.name" class="connection-group">
                                <div class="connection-point-header" :style="getTypeColorStyle(input.type)">
                                    ...
                                    <!-- 代码节点显示删除按钮 -->
                                    <NButton v-if="isCodeNode" quaternary circle size="small"
                                        @click="removeInputPort(input.name)" class="remove-port-button">
                                        <template #icon>
                                            <NIcon>
                                                <RemoveOutline />
                                            </NIcon>
                                        ...
                                        <div class="connection-info">
                                            ...
                                            <div class="connection-details">
                                                <div class="connection-source">
                                                    ...
                                                    <span class="connection-handle">{{ conn.handleLabel || '-'
                                                        }}</span>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="port-management">
                                <div v-if="isCodeNode && !inputEditMode">
                                    <NButton type="default" size="small" @click="inputEditMode = !inputEditMode"
                                ...
                                <div v-if="isCodeNode && inputEditMode" class="inline-form">
                                    <div class="inline-form-row">
                                        <NInput v-model:value="newInputData.name" placeholder="输入参数名（英文）" size="small"
                                            class="inline-form-input" />
                                        <NInput v-model:value="newInputData.label" placeholder="显示名称（可选）" size="small"
                                            class="inline-form-input" />
                                    </div>
                                    <div class="inline-form-row">
                                        <NSelect v-model:value="newInputData.type" size="small"
                                            class="inline-form-input" :options="getTypeOptions()" />
                                    ...
                                    <NSpace>
                                        <NButton type="primary" size="small" @click="addInputPort"
                                            :disabled="!newInputData.name" class="inline-form-button">
                                            添加
                                        </NButton>
                            ...
                            <NEmpty
                                v-if="(!isCodeNode && (!selectedNode.data.blockType.inputs || selectedNode.data.blockType.inputs.length === 0))"
                                description="无输入端口" class="empty-ports">
                                <template #icon>
                    ...
                    <NCollapseItem title="输出连接" name="2">
                        <div v-if="selectedNode">
                            <div v-for="output in selectedNode.type === 'code' ? selectedNode.data.outputs : selectedNode.data.blockType.outputs"
                                :key="output.name" class="connection-group">
                                <div class="connection-point-header" :style="getTypeColorStyle(output.type)">
                                    ...
                                    <!-- 代码节点显示删除按钮 -->
                                    <NButton v-if="isCodeNode" quaternary circle size="small"
                                        @click="removeOutputPort(output.name)" class="remove-port-button">
                                        <template #icon>
                                            <NIcon>
                                                <RemoveOutline />
                                            </NIcon>
                            ...
                            <div class="port-management">
                                <div v-if="isCodeNode && !outputEditMode">
                                    <NButton type="default" size="small" @click="outputEditMode = !outputEditMode"
                                        class="add-port-button">
                                        <template #icon>
                                            <NIcon>
                                                <AddOutline />
                                ...
                                <div v-if="isCodeNode && outputEditMode" class="inline-form">
                                    <div class="inline-form-row">
                                        <NInput v-model:value="newOutputData.name" placeholder="输出参数名（英文）" size="small"
                                            class="inline-form-input" />
                                        <NInput v-model:value="newOutputData.label" placeholder="显示名称（可选）" size="small"
                                            class="inline-form-input" />
                                    </div>
                                    <div class="inline-form-row">
                                        <NSelect v-model:value="newOutputData.type" size="small"
                                            class="inline-form-input" :options="getTypeOptions()" />
                                    </div>
                                    <NSpace>
                                        <NButton type="primary" size="small" @click="addOutputPort"
                                            :disabled="!newOutputData.name" class="inline-form-button">
                                            添加
                                        </NButton>
                                        <NButton size="small" @click="outputEditMode = false"
                            ...
                            <NEmpty
                                v-if="(!isCodeNode && (!selectedNode.data.blockType.outputs || selectedNode.data.blockType.outputs.length === 0))"
                                description="无输出端口" class="empty-ports">
                                <template #icon>
                                    <NIcon size="48" class="empty-icon">
                                        <RemoveOutline />
                                    </NIcon>
                    ...
                    <!-- 普通节点的参数配置 -->
                    <NCollapseItem
                        v-if="selectedNode?.data?.blockType?.configs && selectedNode.data.blockType.configs.length > 0 && !isCodeNode"
                        title="参数配置" name="3">
                        <div v-for="config in selectedNode.data.blockType.configs" :key="config.name"
                            ...
                            <NInput v-if="config.type === 'str' && !config.has_options"
                                ...
                                :placeholder="`请输入${config.label || config.name}`" type="textarea" class="custom-input"
                            ...
                            <NSwitch v-else-if="config.type === 'bool'" v-model:value="formValue[config.name]"
                                class="custom-switch" @update:value="applyNodeConfig" />
                        ...
                        <div class="code-editor-section">
                            <div class="code-editor-container">
                                <div class="code-editor-header">
                                    <NSpace>
                                        <NButton size="small" @click="toggleFullscreenEditor"
                                ...
                                <Editor v-model="formValue.code" language="python" :options="{
                                    minimap: { enabled: false },
                                    lineNumbers: 'on',
                                    scrollBeyondLastLine: false,
                                    automaticLayout: true
                                }" ref="codeEditorRef" @editorDidMount="handleEditorMount" @update:modelValue="applyNodeConfig" />
                            </div>
                        </div>
                    </NCollapseItem>
                    <!-- 全屏编辑器模式 -->
                    <Teleport to="body" v-if="isEditorFullscreen">
                        <div class="fullscreen-editor-container">
                            <div class="fullscreen-editor-header">
                                ...
                                <NSpace>
                                    <NButton size="small" @click="toggleFullscreenEditor" class="editor-action-button">
                                        <template #icon>
                                            <NIcon>
                                                <CloseOutline />
                                            </NIcon>
