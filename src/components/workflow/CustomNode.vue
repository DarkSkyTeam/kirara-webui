...
<template>
    <div class="custom-node">
        ...
        <div class="custom-node-body">
            <div v-if="data.blockType.configs && data.blockType.configs.length > 0" class="config-preview">
                <div v-for="config in data.blockType.configs" :key="config.name" class="config-preview-item">
                    <div class="config-name">{{ config.label || config.name }}</div>
                    <div class="config-value">
                        <!-- 布尔类型配置预览 -->
                        <template v-if="config.type === 'bool'">
                            <span class="config-badge" :class="data.config?.[config.name] ? 'config-badge-true' : 'config-badge-false'">
