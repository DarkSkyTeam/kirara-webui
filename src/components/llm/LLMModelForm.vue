            ...
            <n-form-item label="模型类型" path="type" required>
                <n-space>
                    <n-tooltip trigger="hover" placement="top">
                        <template #trigger>
                            <n-button :type="currentModel.type === 'llm' ? 'primary' : 'default'"
                    ...
                    <n-tooltip trigger="hover" placement="top">
                        <template #trigger>
                            <n-button :type="currentModel.type === 'embedding' ? 'primary' : 'default'"
                    ...
                    <n-tooltip trigger="hover" placement="top">
                        <template #trigger>
                            <n-button :type="currentModel.type === 'imagegeneration' ? 'primary' : 'default'"
                                @click="currentModel.type = 'imagegeneration'" disabled>
                                画图模型
                            </n-button>
                        </template>
                        用于生成和编辑图像的模型，暂不支持
                ...
                <n-space vertical>
                    <n-checkbox-group v-model:value="selectedAbilities" @update:value="updateModelAbility">
                        <n-grid :cols="3" :x-gap="12" :y-gap="12"
                            ...
                            <n-grid-item v-for="ability in modelAbilities[currentModel.type]" :key="ability.value">
